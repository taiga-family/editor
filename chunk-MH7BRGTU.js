...
var Gapcursor = Extension.create({
  ...
  extendNodeSchema(extension) {
    var _callOrReturn;
    return {
      allowGapCursor: (_callOrReturn = callOrReturn(getExtensionField(extension, "allowGapCursor", {
        name: extension.name,
...
function mergeRanges(ranges) {
  ...
  const merged = [__spreadValues({}, sorted[0])];
  for (let i = 1; i < sorted.length; i += 1) {
    const last = merged[merged.length - 1];
    const current = sorted[i];
    if (current.from <= last.to) last.to = Math.max(last.to, current.to);
    else merged.push(__spreadValues({}, current));
  }
  return merged;
}
...
function createPlaceholderPlugin({
  ...
  return new Plugin(__spreadProps(__spreadValues({
    key: PLUGIN_KEY
  }, useResolvedPath ? {} : {
...
//# sourceMappingURL=chunk-MH7BRGTU.js.map
