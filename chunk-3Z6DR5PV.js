...
editHandlers.keypress = (view, _event) => {
  let event = _event;
  if (inOrNearComposition(view) || !event.charCode || event.ctrlKey && !event.altKey || mac2 && event.metaKey) return;
...
var CommandManager = class CommandManager2 {
  ...
  */
  ...
  createCan(startTr) {
    ...
    return __spreadProps(__spreadValues({}, Object.fromEntries(Object.entries(rawCommands).map(([name, command2]) => {
...
var insertDefaultBlock = (options = {}) => ({
  tr,
  dispatch,
  editor
}) => {
  const {
    ...
    attrs,
    content,
    updateSelection: updateSelection2 = true
  } = options;
  let $pos;
...
var joinTextblockBackward$1 = () => ({
  state,
  dispatch
}) => {
  return joinTextblockBackward(state, dispatch);
};
var joinTextblockForward$1 = () => ({
  state,
  dispatch
}) => {
...
var lift$1 = (typeOrName, attributes = {}) => ({
  state,
  dispatch
}) => {
...
var liftEmptyBlock$1 = () => ({
  state,
  dispatch
}) => {
  return liftEmptyBlock(state, dispatch);
};
var liftListItem$1 = (typeOrName) => ({
  state,
  dispatch
}) => {
...
var selectNodeBackward$1 = () => ({
  state,
  dispatch
}) => {
  return selectNodeBackward(state, dispatch);
};
var selectNodeForward$1 = () => ({
  state,
  dispatch
}) => {
  return selectNodeForward(state, dispatch);
};
var selectParentNode$1 = () => ({
  state,
  dispatch
}) => {
  return selectParentNode(state, dispatch);
};
var selectTextblockEnd$1 = () => ({
  state,
  dispatch
}) => {
  return selectTextblockEnd(state, dispatch);
};
var selectTextblockStart$1 = () => ({
  state,
  dispatch
}) => {
...
function getExtensionField(extension, field, context) {
  ...
  if (typeof extension.config[field] === "function") return extension.config[field].bind(__spreadProps(__spreadValues({}, context), {
...
function getSchemaByResolvedExtensions(extensions, editor) {
  ...
  return new Schema({
    ...
    nodes: Object.fromEntries(nodeExtensions.map((extension) => {
      ...
      const schema = cleanUpSchemaItem(__spreadProps(__spreadValues({}, extensions.reduce((fields, e) => {
...
function getDebugJSON(node, startOffset = 0) {
  ...
  const marks = node.marks.map((mark) => {
    ...
    if (Object.keys(mark.attrs).length) output2.attrs = __spreadValues({}, mark.attrs);
    return output2;
  });
  ...
  if (dispatch) if (empty2) {
    ...
    state.doc.nodesBetween(from, to, (node, pos) => {
      ...
      if (node.marks.find((mark) => mark.type === type)) node.marks.forEach((mark) => {
        if (type === mark.type) tr.addMark(trimmedFrom, trimmedTo, type.create(__spreadValues(__spreadValues({}, mark.attrs), attributes)));
      });
...
var setNode = (typeOrName, attributes = {}) => ({
  ...
  return chain().command(({
    ...
    if (setBlockType2(type, __spreadValues(__spreadValues({}, attributesToCopy), attributes))(state)) return true;
  ...
  tr.selection.ranges.forEach((range) => {
    ...
    if (tr.selection.empty) state.doc.nodesBetween(from, to, (node, pos) => {
      ...
      if (pos >= from && pos <= to) {
        if (nodeType && nodeType === node.type) {
          canUpdate = true;
          if (dispatch) tr.setNodeMarkup(pos, void 0, __spreadValues(__spreadValues({}, node.attrs), attributes));
        }
        if (markType && node.marks.length) node.marks.forEach((mark) => {
          if (markType === mark.type) {
            canUpdate = true;
    ...
    if (lastNode) {
      if (lastPos !== void 0 && dispatch) tr.setNodeMarkup(lastPos, void 0, __spreadValues(__spreadValues({}, lastNode.attrs), attributes));
      if (markType && lastNode.marks.length) lastNode.marks.forEach((mark) => {
        if (markType === mark.type && dispatch) tr.addMark(trimmedFrom, trimmedTo, markType.create(__spreadValues(__spreadValues({}, mark.attrs), attributes)));
      });
    }
  });
  return canUpdate;
...
var updateDecorations = (extensionName) => ({
  tr,
  dispatch
}) => {
...
function getRebuildRanges(tr, doc3) {
  ...
  for (const range of ranges) {
    ...
    else merged.push(__spreadValues({}, range));
  }
  return {
    type: "ranges",
    ranges: merged
...
var DecorationManager = class {
  ...
  */
  ...
  warnDuplicateWidgetKeys(state) {
    ...
    const nextWarningKeys = new Set(duplicateKeys.map(({
      key
    }) => key));
    for (const {
      key,
...
function createAtomBlockMarkdownSpec(options) {
  const {
    nodeName,
    name: markdownName,
    parseAttributes: parseAttributes$2 = parseAttributes,
    serializeAttributes: serializeAttributes$2 = serializeAttributes,
    defaultAttributes = {},
    requiredAttributes = [],
    allowedAttributes
  } = options;
...
function createBlockMarkdownSpec(options) {
  const {
    ...
    name: markdownName,
    getContent,
    parseAttributes: parseAttributes$1 = parseAttributes,
    serializeAttributes: serializeAttributes$1 = serializeAttributes,
    defaultAttributes = {},
    content = "block",
    allowedAttributes
  } = options;
    ...
    return [new Plugin({
      ...
      props: {
        clipboardTextSerializer: () => {
          ...
          const options = __spreadProps(__spreadValues({}, blockSeparator !== void 0 ? {
            blockSeparator
          } : {}), {
            textSerializers
          });
...
var InlineDecoration = class extends Decoration2 {
  ...
  toPMDecoration(extensionName) {
    const spec = extensionName ? __spreadProps(__spreadValues({}, this.spec), {
      extensionName
    }) : this.spec;
...
var WidgetDecoration = class extends Decoration2 {
  ...
  toPMDecoration(extensionName) {
    const spec = extensionName ? __spreadProps(__spreadValues({}, this.spec), {
      key: this.key,
      extensionName
    }) : __spreadProps(__spreadValues({}, this.spec), {
      key: this.key
    });
...
function flushPendingProps(cache) {
  ...
  for (const [key, props] of cache.pendingProps) {
    ...
    if (renderer) {
      ...
      cache.props.set(key, __spreadValues({}, props));
    }
  }
...
function createWidgetDecoration(options) {
  ...
  const render2 = (_view, getPos) => {
    const renderProps = __spreadValues(__spreadValues({}, props), context(getPos));
...
var ResizableNodeView = class {
  ...
  */
  constructor(options) {
    ...
    if ((_options$options = options.options) === null || _options$options === void 0 ? void 0 : _options$options.min) this.minSize = __spreadValues(__spreadValues({}, this.minSize), options.options.min);
...
var NodeView = class {
  ...
  stopEvent(event) {
    ...
    if ((["INPUT", "BUTTON", "SELECT", "TEXTAREA"].includes(target.tagName) || target.isContentEditable) && !isDropEvent && !isDragEvent) return true;
    const {
      isEditable
    } = this.editor;
    const {
...
//# sourceMappingURL=chunk-3Z6DR5PV.js.map
